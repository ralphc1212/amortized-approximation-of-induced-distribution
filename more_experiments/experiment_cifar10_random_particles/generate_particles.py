...
ood_loader = torch.utils.data.DataLoader(oodset, batch_size=256, shuffle=False, num_workers=8)
...
with torch.no_grad():
    all_samples = []
    for i in range(100):
        print('checkpoint/ckpt'+str(i)+'.pth')
        net.load_state_dict(torch.load('checkpoint/ckpt'+str(i)+'.pth')['net'])
        samples_a_round = []
