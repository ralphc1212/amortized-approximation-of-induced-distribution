...
def train_approx(args, fmodel, gmodel, device, approx_loader, f_optimizer, g_optimizer, output_samples, epoch):
    ...
    for batch_idx, (data, target) in enumerate(approx_loader):
        ...
        with torch.no_grad():
            ...
            output = F.softmax((output_samples[batch_idx * approx_loader.batch_size:(batch_idx + 1) * approx_loader.batch_size].to(device)), dim=2).clamp(0.0001, 0.9999)
        ...
        with torch.no_grad():
            output = F.softmax((output_samples[batch_idx * approx_loader.batch_size:(batch_idx + 1) * approx_loader.batch_size].to(device)) / temperature, dim=2).clamp(0.0001, 0.9999)
...
def eval_approx(args,  smean, sconc, device, test_loader,
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            samples_p_pi = F.softmax(teacher_test_samples[batch_idx*test_loader.batch_size: (batch_idx+1)*test_loader.batch_size], dim=2).to(device)
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(ood_loader):
            ...
            samples_p_pi = F.softmax(teacher_ood_samples[batch_idx*ood_loader.batch_size: (batch_idx+1)*ood_loader.batch_size], dim=2).to(device)
...
def main():
    ...
    # print(args.model_path + 'ckpt0.pth')
