...
def main():
    ...
    if args.from_approx_model == 0:
        ...
        for epoch in range(1, args.approx_epochs + 1):
            ...
            if acc > best_acc:
                torch.save(fmodel.state_dict(), args.model_path + 'sgld-mnist-mean.pt')
                torch.save(gmodel.state_dict(), args.model_path + 'sgld-mnist-conc.pt')
                best_acc = acc

    else:
        fmodel.load_state_dict(torch.load(args.model_path + 'sgld-mnist-mean.pt'))
        gmodel.load_state_dict(torch.load(args.model_path + 'sgld-mnist-conc.pt'))
